...
(function (w) {
  ...
  async function runActions(actions = []) {
    for (const a of actions) {
      ...
      if (a.type === "navigate") {
        ...
        else announce("Blocked cross-origin navigation.");
      }
      else if (a.type === "click") {
        let el = a.selector ? document.querySelector(a.selector) : null;
        if (!el && a.text) el = findByText(a.text);
        if (el) { announce(a.announce || `Clicking ${a.text || a.selector || ""}`); el.click(); await delay(50); }
        ...
        if (el) {
          ...
          announce(a.announce || "Filled input."); await delay(20);
        }
      }
        ...
        announce(a.announce || "Scrolling."); await delay(80);
      }
  ...
  // Optional recorder helpers (kept minimal)
  let recording = null;
  ...
  function onClick(e) { if (!recording) return; const t = e.target; const label = (t.innerText || t.textContent || "").trim().slice(0, 100); recording.steps.push({ action: "clickText", text: label || "Submit" }); }
  function onKey(e) { if (!recording) return; if (["Enter","Escape","Tab"].includes(e.key)) recording.steps.push({ action: "press", key: e.key }); }

  async function runInCloud(steps, opts = {}) {
    const payload = { browser_task: { startUrl: opts.startUrl || location.href, steps: steps || [], timeoutMs: opts.timeoutMs || 45000, viewport: opts.viewport || { width: 1280, height: 900 } } };
    let json = null;
    if (sb) {
      const { data, error } = await sb.functions.invoke("assistant", { body: payload });
    ...
    return json; // passthrough result
  }
