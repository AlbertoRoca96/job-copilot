...
(async function () {
  ...
  // --- Drafts (signed URLs) ---
  function allowed(type, rel) {
    if (type === "cover") return /\.md$/i.test(rel);
    if (type === "resume") return /\.docx$/i.test(rel); // ONLY docx
    if (type === "changes") return /\.json$/i.test(rel); // ONLY json
    return false;
  }
  ...
  async function loadDrafts() {
    ...
    const seen = new Set(); // avoid dupes
    let rendered = 0;

    for (const [type, rel] of rows) {
      const keyRel = `${type}|${rel}`;
      if (seen.has(keyRel)) continue;
      seen.add(keyRel);

      if (!allowed(type, rel)) continue;
      ...
      if (e2 || !s2?.signedUrl) continue; // skip dead entries
    ...
    if (rendered > 0) {
      ...
      // stop polling once we have any rows
      if (pollTimer) {
  ...
  // Polling after queue (stop automatically once populated)
  let pollTimer = null;
  function pollDrafts() {
