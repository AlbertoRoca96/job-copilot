...
# ---------- JD processing ----------
_HEADERS = {
    # Robust UA/header usage to avoid “bot” blocks; Requests supports custom headers. 
    # See: requests UA guidance. 
...
def pick_jd_text(job_obj: dict) -> str:
    # Prefer the cached description (crawler), but if it’s short, try the live page
...
def main(top: int, user: Optional[str]):
    ...
    for j in jobs:
        ...
        # COVER (generic template; passes jd_keywords to template)
        cover_fname = f"{slug}.md"
        cover = render_cover(j, PROFILE_YAML, TMPL_DIR, jd_keywords=jd_kws)
        ...
        llm_summary["jobs"].append({"slug": slug, "injected": True, "changes": len(explain["changes"])})
