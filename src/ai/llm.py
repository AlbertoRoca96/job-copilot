...
# ----------------------- NEW: cover letter section composer -----------------------
def craft_cover_sections(api_key: str,
                         model: str,
                         job_title: str,
                         company: str,
                         jd_text: str,
    ...
    """
    Compose a company- and JD-specific cover letter as JSON sections (≤400 words total).
      - No fabrication of metrics. Use neutral scope language if numbers are unknown.
      - 3–4 short paragraphs + optional bullet alignment block.
      - Natural, jargon-light, cross-industry friendly.
    """
    if not api_key:
        return {}
    ...
    # guardrails
    banlist = [
        "I am writing to express my interest",  # cliché opener
        "Dear Sir or Madam", "To whom it may concern",
        "dynamic", "synergy", "self-starter", "rockstar"
    ]

    system = (
        "You are an expert career writer who crafts concise, high-signal cover letters for ANY industry. "
        "Write with clarity and restraint; avoid buzzwords; do not invent metrics. "
        "Output JSON with fields: greeting, opening_hook, evidence_1, evidence_2, company_alignment (array of 2-5), "
        "closing, signature. Keep total length ≤400 words."
    )

    user = json.dumps({
        "job_title": job_title, "company": company,
        "job_description": _trim(jd_text, 6000),
        "profile": {
    ...
    obj = _openai_json(api_key, model, system, user, expect="object", max_tokens=900, temperature=0.3) or {}
    # sanitize & trim
    out = {
        "greeting": _collapse_ws(str(obj.get("greeting") or f"Dear Hiring Team at {company},")),
        "opening_hook": _collapse_ws(str(obj.get("opening_hook") or "")),
        "evidence_1": _collapse_ws(str(obj.get("evidence_1") or "")),
        "evidence_2": _collapse_ws(str(obj.get("evidence_2") or "")),
        "company_alignment": [ _collapse_ws(str(x)) for x in (obj.get("company_alignment") or []) if x ][:5],
        "closing": _collapse_ws(str(obj.get("closing") or "Thank you for your time and consideration.")),
        "signature": _collapse_ws(str(obj.get("signature") or (
            (profile.get("full_name") or "") + ("\n" + " | ".join([x for x in [profile.get('email'), profile.get('phone')] if x]) if (profile.get("email") or profile.get("phone")) else "")
        )))
    }
    # lightweight banlist filter
    low = json.dumps(out).lower()
    if any(b in low for b in banlist):
        # Nuke clichés if they slipped through.
        out["opening_hook"] = out["opening_hook"].replace("I am writing to express my interest", "").strip()
    return out
